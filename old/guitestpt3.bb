...
Dim vaultalarm(4):vaultalarm(1)=1:vaultalarm(2)=1:vaultalarm(3)=1:vaultalarm(4)=1
...
If anngo=1
	...
	If Int(EntityX#(mru))=Int(EntityX(ann))+1 Or Int(EntityX#(mru))=Int(EntityX(ann))-1
		If Int(EntityZ#(mru)) = Int(EntityZ#(ann)) Then Gosub StopMRU
		;Stop and take control away from mru
	ElseIf Int(EntityZ#(mru))=Int(EntityZ(ann))+1 Or Int(EntityZ#(mru))=Int(EntityZ(ann))-1
...
If destmru=1
	;Destroy the mru
	HideEntity mru
	If MilliSecs() > alarmtime+5000 ; Once 5 seconds have passed (or however long it takes to destroy the MRU)
...
;Various vholdspeed# and vholdpos# operations
	...
	Text 0,0,mrudirection
	Text 0,20,mruperformaction
