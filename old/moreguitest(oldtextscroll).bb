...
;NOSECURITY - security cameras and guard can't see the MRU!
;TIPTOPMRU - MRU doesn't suffer any malfunctions during mission

Graphics3D 640,480,32,2;0
...
textmult#=FL\SpeedFactor


textspeed#=(textmult# * (-7.8+(textmult#*textmult#))); * (textmult#/textmult#);-2.5 * (textmult#/textmult#)
textspace#=(textmult# * (5.2+(textmult#*textmult#))) * textmult#;1.5 * textmult#
textstop#=8 * textmult#
...
EndIf ;If the 'no secutity' cheat is not on
...
ElseIf Int(EntityX#(mru))=12 And Int(EntityZ#(mru))=-16 And mrudirection=1 And mruperformaction=0 ;Terminal D
	a=0 : b=282 : guigo=0 : textdisplay=1 : showcursor=0 : mruterminal=4 : If stage1=1 And stage2=1 And stage3=1 Then stage4=1
...
;Checking position of the mru to see if faults need to occur
...
If millisecscount#>time#+(typepause# * FL\SpeedFactor)
	...
	ElseIf msg(b,1)="VAULTYNCHECK"
		If vaultyn$="Y"
			...
			If vaultalarmnum=1 Then vaultalarm(1)=1 : stage1=1 : stage2=1 : stage3=1 : stage4=1 : stage5=1 : a=0 : b=324 : If stage1distance=0 Then stage1distance=mrudistance
			If vaultalarmnum=2 Then If vaultalarm(1)=1 Then vaultalarm(2)=1 : stage6=1 : a=0 : b=324 Else mruspotted=1 : whosawmru=0 : alarmtime#=millisecscount# : a=0 : b=231 ;Set off alarm and set all switches to off
			...
			If vaultalarmnum=4 Then If vaultalarm(1)=1 And vaultalarm(2)=1 And vaultalarm(3)=1 Then vaultalarm(4)=1 : stage8=1 : a=0 : b=324 Else mruspotted=1 : whosawmru=0 : alarmtime#=millisecscount# : a=0 : b=231 ;Set off alarm
...
If MouseX()>256 And MouseX()<304 And MouseY()>455 And MouseY()<468 And mouseclick=0 Then buttonpress=9 : If camfunc(monselected)<>4 : clocksecspeed(monselected)=1 : clockspeed(monselected)=2 : vtrstopped(monselected)=0 : Gosub normalclock : camfunc(monselected)=4 : cambypass(monselected)=0 : Gosub camchangetomap : vholdpos#(monselected)=110 : tx$(monselected)=tx$(monselected)+textscroll$(mrudirection) : scrollgo(monselected)=1 : EndIf : mouseclick=1 ;TGS
...
If doorvar=1 ;If an open door is wanted
	...
	AddVertex ( v,xpos#,wallheight#,zpos#-z#) ;0
	AddVertex ( v,xpos#,wallheight#,zpos#) ;1
	AddVertex ( v,xpos#,0,zpos#) ;2
	AddVertex ( v,xpos#,0,zpos#-doorstart#) ;3
	AddVertex ( v,xpos#,doorheight#,zpos#-doorstart#) ;4
	AddVertex ( v,xpos#,doorheight#,zpos#-doorstart#-doorwidth#) ;5
	AddVertex ( v,xpos#,0,zpos#-doorstart#-doorwidth#) ;6
	AddVertex ( v,xpos#,0,zpos#-z#) ;7
	;Other side of the wall
	AddVertex ( v,xpos#+x#,wallheight#,zpos#-z#) ;8
	AddVertex ( v,xpos#+x#,wallheight#,zpos#) ;9
	AddVertex ( v,xpos#+x#,0,zpos#) ;10
	AddVertex ( v,xpos#+x#,0,zpos#-doorstart#) ;11
	AddVertex ( v,xpos#+x#,doorheight#,zpos#-doorstart#) ;12
	AddVertex ( v,xpos#+x#,doorheight#,zpos#-doorstart#-doorwidth#) ;13
	AddVertex ( v,xpos#+x#,0,zpos#-doorstart#-doorwidth#) ;14
	AddVertex ( v,xpos#+x#,0,zpos#-z#) ;15
...
ElseIf doorvar=0 ;Otherwise, build a plain wall with no door
	...
	AddVertex ( v,xpos#,wallheight#,zpos#) ;1
	AddVertex ( v,xpos#,0,zpos#) ;2
	AddVertex ( v,xpos#,0,zpos#-z#) ;3
	;Other side of the wall
	AddVertex ( v,xpos#+x#,wallheight#,zpos#-z#) ;4
	AddVertex ( v,xpos#+x#,wallheight#,zpos#) ;5
	AddVertex ( v,xpos#+x#,0,zpos#) ;6
	AddVertex ( v,xpos#+x#,0,zpos#-z#) ;7
	;Build left side of wall
	AddTriangle( v,0,2,1)
	AddTriangle( v,0,3,2)
	;Build right side of wall
